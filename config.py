...
# CORS: comma-separated; production should set explicit origins
...
MAX_DURATION_SECONDS = _int("MAX_DURATION_SECONDS", 4 * 60 * 60)  # 4 hours
MAX_FILE_BYTES = _int("MAX_FILE_BYTES", 3 * 1024 * 1024 * 1024)  # 3 GiB
MIN_FREE_DISK_BYTES = _int("MIN_FREE_DISK_BYTES", 512 * 1024 * 1024)  # 512 MiB

RATE_LIMIT_INFO = os.environ.get("RATE_LIMIT_INFO", "30/minute")
RATE_LIMIT_JOBS = os.environ.get("RATE_LIMIT_JOBS", "8/hour")

# Launch scope: primary extractor (yt-dlp supports more; we advertise YouTube v1)
SUPPORTED_URL_HOSTS = [
    h.strip().lower()
...
# When behind a reverse proxy, trust X-Forwarded-For for per-IP limits (set only if proxy strips spoofed headers)
